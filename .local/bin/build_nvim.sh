...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -s, --stable  Build the latest stable release instead of master"
...
# Determine target ref based on build mode
if [ "$BUILD_STABLE" = true ]; then
    # Get latest stable tag (excludes pre-release tags like v0.10.0-dev)
    TARGET_REF=$(git tag -l 'v[0-9]*.[0-9]*.[0-9]' --sort=-v:refname | head -n1)
    if [ -z "$TARGET_REF" ]; then
        echo "ERROR: Could not find any stable release tags"
        exit 1
    fi
    TARGET_COMMIT=$(git rev-parse "$TARGET_REF")
    echo "Latest stable release: $TARGET_REF"
else
...
if [ "$CURRENT_COMMIT" = "$TARGET_COMMIT" ] && [ "$FORCE_REBUILD" = false ]; then
...
if [ "$CURRENT_COMMIT" != "$TARGET_COMMIT" ] && [ -d ".deps" ]; then
    echo "Cleaning .deps..."
    rm -rf .deps
fi
